...
class ContractMirrorService:
    ...
    def __init__(self, rpc_url: str, deployment_path: str, contract_name: str = "ClinicalTrialRegistry"):
        ...
        self.web3 = Web3(Web3.HTTPProvider(self.rpc_url, request_kwargs={"timeout": 30})) if Web3 else None
        self.contract = None
    ...
    def health(self) -> Dict[str, Any]:
        return {
            ...
            "connected": bool(self.web3 and self.web3.is_connected()),
    ...
    def _tx_params(self, from_address: str) -> Dict[str, Any]:
        if self.web3 is None:
            raise RuntimeError("web3 is unavailable")
        return {
    ...
    def _send_contract_call(self, fn, from_address: str, private_key: Optional[str] = None) -> Dict[str, Any]:
        if self.web3 is None:
            return {"ok": False, "reason": "web3 unavailable"}
        if private_key:
